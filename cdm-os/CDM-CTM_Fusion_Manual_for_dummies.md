...
### 4. How to Set It Up (Zero-to-Running in 10 Minutes)
Needs: Free Ollama (download at ollama.com) + a decent GPU (RTX 4070 or Mac M3). Installs in one command.

1. **Install Basics** (Terminal/CMD):
   ```
   pip install ollama transformers torch numpy accelerate
   ollama pull llama3.1:8b  # Or your favorite model
   ```

2. **Save as `fusion.py`** (Copy-paste this file):
   ```
   # fusion.py — CDM-CTM Feedback Loop (Local AI Coach)
   import ollama
   import torch
   import numpy as np
   from transformers import AutoModelForCausalLM, AutoTokenizer

   # CDM v2 (measures depth)
   # [Paste the full CDM v2 code from previous messages here — ~80 lines]

   class CDM_CTM_Fusion:
       def __init__(self, model_name="llama3.1:8b", target_cdm=70, base_max_ctm=256):
       ...
       def fusion_generate(self, prompt):
           ...
           while thinking_steps < max_ctm:
               ...
               print(f"Step {thinking_steps}: CDM = {cdm_value} ({label}), Change = {delta_cdm}")

               if cdm_value >= self.target_cdm and label == "deep CRYSTAL":
                   break

               if delta_cdm > 5 and thinking_steps > max_ctm * 0.75:
                   max_ctm += 256
                   print(f"Extending time to {max_ctm} (good progress)")

               self.granularity = min(3, self.granularity + 0.5 if delta_cdm < 2 else self.granularity)

               cot_prompt = f" <think step, level {self.granularity}>"  # Simple adjustment
   ...
   if __name__ == "__main__":
       ...
       response, final_cdm = fusion.fusion_generate("Plan a 5-year learning goal. Think step by step.")
       print(f"\nFinal Plan: {response}\nFinal Depth: {final_cdm}")
   ```
...
4. **What You See**:
   - Prints steps like "Step 5: CDM = 45 (shallow), Change = 12"
   - Adds time if progress is good.
   - Final output: A deeper plan + final score.

### 5. Customize It (For Fun)
- Change `target_cdm=80` for harder thinking.
- Add your prompts — watch CDM rise.
